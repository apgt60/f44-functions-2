...
// CODE HERE
const multiply = function(num1, num2, callbackFn){
...
// CODE HERE 
const first = (inputArr, callbackFn) => callbackFn(inputArr[0])
...
const last = (inputArr, callbackFn) => {
  callbackFn(inputArr[inputArr.length - 1])
}
...
const contains = (inputArr, name, callbackFn) => {
...
contains(names, 'Dale', result => {
  if(result === true){
    console.log('Dale is in the array')
  } else {
    console.log('Dale is not in the array')
  }
})
...
const uniq = (inputArr, callbackFn) => {
...
const each = (inputArr, callbackFn) => {
  for(let i=0; i<inputArr.length; i++){
    callbackFn(inputArr[i], i)
  }
}
...
const addingFactory = number => anotherNumber => number + anotherNumber
...
console.log("addNUMBER(4)", addNUMBER(4))
...
/*
  ...
  You'll learn about objects in our next lecture. But you can research objects and attempt this 
  problem.
*/
...
const getUserById = (users, userId, callbackFn) => {
  let matchingObject = null
  for(let i=0; i < users.length; i++){
    if(users[i].id === userId){
      matchingObject = users[i]
    }
  }
  callbackFn(matchingObject)
}
